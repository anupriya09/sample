...
/**
 * This Class implements Runnable interface to create User dictionary from View
 * map for every USerID and store it in Ignite Cache.
 */
public class ViewRunnable_Enseble implements Runnable {
	public String uid;

	public ViewRunnable_Enseble(String uid) {
	...
	@Override
	public void run() {
		LinkedList<String> listDoc = PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.viewMap.get(uid);
		...
		while (iter.hasNext()) {
			...
			if (str != null) {
				...
				readUserDFN = map_funcView(readUserDF, record);
					...
					for (Long window : PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows) {
						...
						if (diffFromEndD <= window && diffFromEndD >= 0) {
							...
							for (int j = win_index; j < PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.size(); j++) {
								// View/Order Counts
								tempDoc.replace(
										"view_Count_Window" + PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j)
												+ "days",
										(int) tempDoc.get("view_Count_Window"
												+ PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j) + "days") + 1);
								// Session Counts
								tempDoc.replace(
										"view_Count_Session_Window"
												+ PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j) + "days",
										(int) tempDoc.get("view_Count_Session_Window"
												+ PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j) + "days") + 1);
							}
							break;
						}
				...
				if (weekno <= 4) {
					AllTempDoc.replace("view_Count_Weekday", AllTempDoc.getInteger("view_Count_Weekday") + 1);
				}
				// Assigning hour
				int hourno = cal.get(cal.HOUR_OF_DAY);
				if (!((Document) AllTempDoc.get("view_Count_Hour")).containsKey("Hour_" + hourno)) {
					((Document) AllTempDoc.get("view_Count_Hour")).append("Hour_" + hourno, 1);
				} else {
					((Document) AllTempDoc.get("view_Count_Hour")).replace("Hour_" + hourno,
							((Document) AllTempDoc.get("view_Count_Hour")).getInteger("Hour_" + hourno) + 1);
				}
				// Category 1
				String cat1Value = category1;
				if (!((Document) AllTempDoc.get("view_Count_Category1")).containsKey("Category1_" + cat1Value)) {
					((Document) AllTempDoc.get("view_Count_Category1")).append("Category1_" + cat1Value, 1);
				} else {
					((Document) AllTempDoc.get("view_Count_Category1")).replace("Category1_" + cat1Value,
							((Document) AllTempDoc.get("view_Count_Category1")).getInteger("Category1_" + cat1Value)
									+ 1);
				}
				// Category 2
				String cat2Value = category2;
				if (!((Document) AllTempDoc.get("view_Count_Category2")).containsKey("Category2_" + cat2Value)) {
					((Document) AllTempDoc.get("view_Count_Category2")).append("Category2_" + cat2Value, 1);
				} else {
					((Document) AllTempDoc.get("view_Count_Category2")).replace("Category2_" + cat2Value,
							((Document) AllTempDoc.get("view_Count_Category2")).getInteger("Category2_" + cat2Value)
									+ 1);
				}
				// Category 3
				String cat3Value = category3;
				if (!((Document) AllTempDoc.get("view_Count_Category3")).containsKey("Category3_" + cat3Value)) {
					((Document) AllTempDoc.get("view_Count_Category3")).append("Category3_" + cat3Value, 1);
				} else {
					((Document) AllTempDoc.get("view_Count_Category3")).replace("Category3_" + cat3Value,
							((Document) AllTempDoc.get("view_Count_Category3")).getInteger("Category3_" + cat3Value)
									+ 1);
				}
				// Category 4
				String cat4Value = category4;
				if (!((Document) AllTempDoc.get("view_Count_Category4")).containsKey("Category4_" + cat4Value)) {
					((Document) AllTempDoc.get("view_Count_Category4")).append("Category4_" + cat4Value, 1);
				} else {
					((Document) AllTempDoc.get("view_Count_Category4")).replace("Category4_" + cat4Value,
							((Document) AllTempDoc.get("view_Count_Category4")).getInteger("Category4_" + cat4Value)
									+ 1);
				}
				// Device
				String deviceValue = device;
				if (!((Document) AllTempDoc.get("view_Count_Device")).containsKey("Device_" + deviceValue)) {
					((Document) AllTempDoc.get("view_Count_Device")).append("Device_" + deviceValue, 1);
				} else {
					((Document) AllTempDoc.get("view_Count_Device")).replace("Device_" + deviceValue,
							((Document) AllTempDoc.get("view_Count_Device")).getInteger("Device_" + deviceValue) + 1);
				}
		...
		// PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.viewMap.remove(uid);
		// System.out.println(globalDict);

	}

	/**
	 * This method is called internally from class ViewRunnable_Enseble which
	 ...
	 */
	public Document map_funcView(Document globalDict, Document record) {
		ArrayList<Date> dateWindow = new ArrayList<Date>();
		ArrayList<String> strDateWindow = new ArrayList<String>();
		String dateStr = "";
		Calendar cal = Calendar.getInstance();
		...
		for (int i = 0; i < 46; i++) {
			...
			for (Long window : PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows) {
				// Long window = x;
				if (diffFromEndD <= window && diffFromEndD >= -1) {
					int win_index = PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.indexOf(window);
					for (int j = win_index; j < PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.size(); j++) {
						// View/Order Counts
						tempDoc.replace(
								"view_Count_Window" + PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j) + "days",
								(int) tempDoc.get("view_Count_Window"
										+ PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j) + "days") + 1);
						// Session Counts
						tempDoc.replace(
								"view_Count_Session_Window" + PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j)
										+ "days",
								(int) tempDoc.get("view_Count_Session_Window"
										+ PAKDDLSTMCDFHiddenStatesAPI0_Ensemble.windows.get(j) + "days") + 1);
					}
		...
		// Assigning hour
		int hourno = cal.get(cal.HOUR_OF_DAY);
		if (!AllTempDoc.containsKey("view_Count_Hour")) {
			AllTempDoc.append("view_Count_Hour", new Document());
			((Document) AllTempDoc.get("view_Count_Hour")).append("Hour_" + hourno, 1);
		} else {
			if (!((Document) AllTempDoc.get("view_Count_Hour")).containsKey("Hour_" + hourno)) {
				((Document) AllTempDoc.get("view_Count_Hour")).append("Hour_" + hourno, 1);
		...
		// Category 1
		String cat1Value = category1;
		if (!AllTempDoc.containsKey("view_Count_Category1")) {
			AllTempDoc.append("view_Count_Category1", new Document());
			((Document) AllTempDoc.get("view_Count_Category1")).append("Category1_" + cat1Value, 1);
		} else {
			if (!((Document) AllTempDoc.get("view_Count_Category1")).containsKey("Category1_" + cat1Value)) {
				((Document) AllTempDoc.get("view_Count_Category1")).append("Category1_" + cat1Value, 1);
				...
				((Document) AllTempDoc.get("view_Count_Category1")).replace("Category1_" + cat1Value,
						((Document) AllTempDoc.get("view_Count_Category1")).getInteger("Category1_" + cat1Value) + 1);
			}
		}
		// Category 2
		String cat2Value = category2;
		if (!AllTempDoc.containsKey("view_Count_Category2")) {
			AllTempDoc.append("view_Count_Category2", new Document());
			((Document) AllTempDoc.get("view_Count_Category2")).append("Category2_" + cat2Value, 1);
		} else {
			if (!((Document) AllTempDoc.get("view_Count_Category2")).containsKey("Category2_" + cat2Value)) {
				((Document) AllTempDoc.get("view_Count_Category2")).append("Category2_" + cat2Value, 1);
				...
				((Document) AllTempDoc.get("view_Count_Category2")).replace("Category2_" + cat2Value,
						((Document) AllTempDoc.get("view_Count_Category2")).getInteger("Category2_" + cat2Value) + 1);
			}
		}
		// Category 3
		String cat3Value = category3;
		if (!AllTempDoc.containsKey("view_Count_Category3")) {
			AllTempDoc.append("view_Count_Category3", new Document());
			((Document) AllTempDoc.get("view_Count_Category3")).append("Category3_" + cat3Value, 1);
		} else {
			if (!((Document) AllTempDoc.get("view_Count_Category3")).containsKey("Category3_" + cat3Value)) {
				((Document) AllTempDoc.get("view_Count_Category3")).append("Category3_" + cat3Value, 1);
				...
				((Document) AllTempDoc.get("view_Count_Category3")).replace("Category3_" + cat3Value,
						((Document) AllTempDoc.get("view_Count_Category3")).getInteger("Category3_" + cat3Value) + 1);
			}
		}
		// Category 4
		String cat4Value = category4;
		if (!AllTempDoc.containsKey("view_Count_Category4")) {
			AllTempDoc.append("view_Count_Category4", new Document());
			((Document) AllTempDoc.get("view_Count_Category4")).append("Category4_" + cat4Value, 1);
		} else {
			if (!((Document) AllTempDoc.get("view_Count_Category4")).containsKey("Category4_" + cat4Value)) {
				((Document) AllTempDoc.get("view_Count_Category4")).append("Category4_" + cat4Value, 1);
				...
				((Document) AllTempDoc.get("view_Count_Category4")).replace("Category4_" + cat4Value,
						((Document) AllTempDoc.get("view_Count_Category4")).getInteger("Category4_" + cat4Value) + 1);
			}
		}
		// Device
		String deviceValue = device;
		if (!AllTempDoc.containsKey("view_Count_Device")) {
			AllTempDoc.append("view_Count_Device", new Document());
			((Document) AllTempDoc.get("view_Count_Device")).append("Device_" + deviceValue, 1);
		} else {
			if (!((Document) AllTempDoc.get("view_Count_Device")).containsKey("Device_" + deviceValue)) {
				((Document) AllTempDoc.get("view_Count_Device")).append("Device_" + deviceValue, 1);
